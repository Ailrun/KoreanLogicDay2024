...
\AtBeginDocument{%
  ...
  \date{}
  \author[J. Jang]{Junyoung Jang\\[0.2em]\scriptsize{junyoung.jang@mail.mcgill.ca}\\[0.2em]\scriptsize{McGill University}\\[1em]\scriptsize{Joint work with Brigitte Pientka, Sophia Roshal, and Frank Pfenning}}
}
...
\begin{frame}{Adjoint Logic in Action}
  \color{black}
  \textbf{Practical and uniform} foundation for proof/programming about modalities with
  \pause
  \begin{itemize}[<+->]
  \item Decidable type checking
  \item Operational/reduction semantics
  \item Computational interpretation for \(\tyUp{}{}{}\)/\(\tyDown{}{}{}\)
  \item Safety properties
  \item Embedding \(\Box\)/\(\mathtt{next}\)/\(\bigcirc\) modalities\\
    \pause
    while preserving static and dynamic semantics
  \end{itemize}
  \pause
  \centering\large ``\elevator''
  \pause
\end{frame}
...
\begin{frame}{Mode Specification}
  \color{black}
  Modes are members of a {\color{violet}preorder}\\Each mode controls allowed {\color{orange}structural rules} and {\color{blue}types} in its sublogic.\\[1em]
...
pow 0       =
  nb<<store$\color{NavyBlue}^c_p$>>nb (nb<<thunk$\color{NavyBlue}^c_p$>>nb (fun x -> 1))
pow (suc n) =
  nb<<load$\color{NavyBlue}^c_p$>>nb P = pow n nb<<in>>nb
  nb<<store$\color{NavyBlue}^c_p$>>nb (nb<<thunk$\color{NavyBlue}^c_p$>>nb (fun x -> x * ((nb<<force$\color{NavyBlue}^c_p$>>nb P) x))
\end{lstlisting}
    };
...
pow :$^p$ Nat -> nb<<v|$\color{NavyBlue}^c_p$^|$\color{NavyBlue}^c_p$>>nb(Nat -> Nat)
ws<<pow 0       = >>ws
  ws<<store$\color{WildStrawberry}^c_p$ (thunk$\color{WildStrawberry}^c_p$ (fun x -> 1))>>ws
pow (suc n) =
  nb<<load$\color{NavyBlue}^c_p$>>nb P = pow n nb<<in>>nb
  nb<<store$\color{NavyBlue}^c_p$>>nb (nb<<thunk$\color{NavyBlue}^c_p$>>nb (fun x -> x * ((nb<<force$\color{NavyBlue}^c_p$>>nb P) x))
\end{lstlisting}
...
pow :$^p$ Nat -> nb<<v|$\color{NavyBlue}^c_p$^|$\color{NavyBlue}^c_p$>>nb(Nat -> Nat)
pow 0       =
  nb<<store$\color{NavyBlue}^c_p$>>nb (ws<<thunk$\color{WildStrawberry}^c_p$ (fun x -> 1)>>ws)
pow (suc n) =
  nb<<load$\color{NavyBlue}^c_p$>>nb P = pow n nb<<in>>nb
  nb<<store$\color{NavyBlue}^c_p$>>nb (nb<<thunk$\color{NavyBlue}^c_p$>>nb (fun x -> x * ((nb<<force$\color{NavyBlue}^c_p$>>nb P) x))
\end{lstlisting}
...
pow 0       =
  nb<<store$\color{NavyBlue}^c_p$>>nb (nb<<thunk$\color{NavyBlue}^c_p$>>nb (fun x -> 1))
ws<<pow (suc n) = >>ws
  ws<<load$\color{WildStrawberry}^c_p$ P = pow n in>>ws
  ws<<store$\color{WildStrawberry}^c_p$ (thunk$\color{WildStrawberry}^c_p$ (fun x -> x * ((force$\color{WildStrawberry}^c_p$ P) x))>>ws
\end{lstlisting}
    };
...
pow 0       =
  nb<<store$\color{NavyBlue}^c_p$>>nb (nb<<thunk$\color{NavyBlue}^c_p$>>nb (fun x -> 1))
pow (suc n) =
  nb<<load$\color{NavyBlue}^c_p$>>nb P = ws<<pow n>>ws nb<<in>>nb
  nb<<store$\color{NavyBlue}^c_p$>>nb (nb<<thunk$\color{NavyBlue}^c_p$>>nb (fun x -> x * ((nb<<force$\color{NavyBlue}^c_p$>>nb P) x))
\end{lstlisting}
    };
...
pow (suc n) =
  nb<<load$\color{NavyBlue}^c_p$>>nb P = pow n nb<<in>>nb
  nb<<store$\color{NavyBlue}^c_p$>>nb (nb<<thunk$\color{NavyBlue}^c_p$>>nb (fun x -> x * ((ws<<force$\color{WildStrawberry}^c_p$ P>>ws) x))
\end{lstlisting}
    };
...
pow (suc n) =
  nb<<load$\color{NavyBlue}^c_p$>>nb P = pow n nb<<in>>nb
  nb<<store$\color{NavyBlue}^c_p$>>nb (ws<<thunk$\color{WildStrawberry}^c_p$ (fun x -> x * ((force$\color{WildStrawberry}^c_p$ P) x)>>ws)
\end{lstlisting}
    };
...
\begin{frame}{Working with a Protocol}
  \color{black}
  \begin{itemize}[<+->]
  \item Linear types can encode session types
  \item Unrestricted types can encode functional program
  \end{itemize}
  \begin{tikzpicture}[fulloverlay]
...
\begin{frame}{Linear Calculus with \(\Box\) and \(!\) -- Protocol with Remote Execution}
  \begin{tikzpicture}[fulloverlay]
    \node at (0.5,0.5) {
...
\begin{frame}{Lambda Calculus with \(\bigcirc\) \---- Dead Code Analysis}
  \begin{tikzpicture}[fulloverlay]
    \node[right] at (0.33,0.5) {
      \begin{tikzpicture}[every node/.style=right,xscale=0.1,yscale=0.2,every node/.style={scale=1}]
        \draw
        (0,0.5) rectangle (3.4,1)
        (0,0) rectangle (3.4,0.5);
    ...
    \node at (0.5,0.2) {\(\bigcirc A = \tyUp{p}{s}{\tyDown{p}{s}{A}}\)};
  \end{tikzpicture}
\end{frame}

\begin{frame}{Lambda Calculus with \(\mathtt{next}\) \---- Binding Time Analysis}
  \begin{tikzpicture}[fulloverlay]
    \node[right] at (0.33,0.5) {
      \begin{tikzpicture}[every node/.style=right,xscale=0.1,yscale=0.2,every node/.style={scale=1}]
        \draw
        (0,0.5) rectangle (3.4,1)
        (0,0) rectangle (3.4,0.5)
        (0,-0.5) rectangle (3.4,0);
        \node (t0) at (0.1,0.75) {\(t_0\) -};
        \node (t0m) at (0.55,0.75) {\(\color{orange}\{\mathtt{Wk}, \mathtt{Co}\}\)};
        \node (t0o) at (1.65,0.75) {\(, \color{blue}\{\tyArr{}{}, \tyNat\}\)};
        \node (t1) at (0.1,0.25) {\(t_1\) -};
        \node (t1m) at (0.55,0.25) {\(\color{orange}\{\mathtt{Wk}, \mathtt{Co}\}\)};
        \node (t1o) at (1.65,0.25) {\(, \color{blue}\{\tyDown{}{}{}, \tyArr{}{}, \tyNat\}\)};
        \node[centered] at (1.7,-0.25) {\(\ldots\)};
      \end{tikzpicture}
    };

    \node at (0.5,0.2) {\(\mathtt{next}\;A = \tyDown{t_{n}}{t_{n+1}}{A}\)};
  \end{tikzpicture}
\end{frame}
