...
\AtBeginDocument{%
  \title{\elevator\\\normalsize{Adjoint Logic: Multiple Modalities into a Computational Framework}}
  \date{}
...
\begin{frame}{\elevator as a Uniform Framework}
  \color{black}
  \begin{tikzpicture}[fulloverlay]
    \node[visible on=<1->] at (0.3,0.45) {Linear/Affine Types};
    \node[visible on=<2->,alt=<3->{gray}{},text centered,text width=10em] at (0.3,0.37) {\footnotesize Effect Modelling\\[-0.2em]Protocol Modelling};
    ...
    \node[visible on=<8->,WildStrawberry] at (0.5,0.77) {\large \elevator};
...
\begin{frame}{Adjoint Logic}
  \color{black}
  Uniform logic with two adjoint functors \(\tyUp{}{}{}\) (upshift)/\(\tyDown{}{}{}\) (downshift)\\connecting different sublanguages:
  \pause
  \begin{itemize}[<+->]
  ...
  \item \color{red} No syntactic operational semantics
  \item \color{red} No computational interpretation for \(\tyUp{}{}{}\)/\(\tyDown{}{}{}\)
  \end{itemize}
\end{frame}

\begin{frame}{\elevator \textemdash Substructural Calculus with Adjoint Modalities}
  \color{black}
...
\begin{frame}{Overview of \elevator}
  ...
  \begin{center}
    ...
    \begin{tikzpicture}
      ...
      (0,0) rectangle (2,1)
      (2,0) rectangle (4,1);
      \node[alt=<3>{WildStrawberry}{}] (l0) at (1,0.5) {Sublang0};
      \node (l1) at (3,0.5) {Sublang1};
      \node[alt=<3>{WildStrawberry}{}] (l2) at (2,1.5) {Sublang2};
      \node (l3) at (1,2.5) {Sublang3};
      \node (l4) at (3,2.5) {Sublang4};
  ...
  \scriptsize
  \begin{tikzpicture}[fulloverlay]
    \node[visible on=<2>,WildStrawberry,text centered,text width=18em] (desc0) at (0.8,0.75) {There are multiple sublanguages\\each of which resides in one "mode"};
    ...
    \node[visible on=<3>,WildStrawberry,text centered,text width=22em] (desc1) at (0.2,0.29) {Adjoint modalities \(\tyUp{}{}{}\) and \(\tyDown{}{}{}\) allow\\ interaction between sublanguages};
...
\begin{frame}{Mode Specification}
  \color{black}
  Modes are members of a {\color{violet}preorder}\\A mode controls allowed {\color{orange}structural rules} and {\color{blue}types} in a sublanguage.\\[1em]
